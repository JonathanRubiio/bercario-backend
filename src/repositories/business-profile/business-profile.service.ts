...
export class BusinessProfileService {
  ...
  async getLandingConfig(userId: string): Promise<any> {
    ...
    // Si es la primera vez (no tiene landingConfig), interceptamos y guardamos el default por nicho
    if (!profile.landingConfig || profile.landingConfig.length === 0) {
      ...
      // Inyectar valores dinámicos del perfil en la plantilla seleccionada
  ...
  async updateLandingConfig(userId: string, data: any): Promise<any> {
    const profile = await this.profileRepo.findById({ userId });
    if (!profile) {
      throw new NotFoundException('Perfil de negocio no encontrado');
    }

    const sortedConfig = (data.landingConfig || []).sort((a: any, b: any) => a.order - b.order);
